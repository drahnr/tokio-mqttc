...
pub struct PublishRecieveHandler<'p, P>
where
...
impl<'p, P> PublishRecieveHandler<'p, P>
where
    P: 'p + Send + Persistence,
{
    pub fn new(
...
impl<'p, P> Future for PublishRecieveHandler<'p, P>
where
    P: 'p + Send + Persistence,
{
    type Item = ();
                    ...
                    let key = match data.persistence.append(ser) {
                        ...
                        Err(e) => {
                            if let Some(sender) = sender {
                                let _ =
                                    sender.send(Err(e).chain_err(|| ErrorKind::PersistenceError));
                            }
                            return Err(ErrorKind::PersistenceError.into());
                        }
                    };
                    let _ = data.client_publish_state.insert(
                        *id,
                        PublishState::Released(key, sender),
                    );
