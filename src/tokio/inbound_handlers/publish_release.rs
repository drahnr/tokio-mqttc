...
pub struct PublishReleaseHandler<'p, P>
where
...
impl<'p, P> PublishReleaseHandler<'p, P>
where
    P: 'p + Send + Persistence,
{
    pub fn new(
...
impl<'p, P> Future for PublishReleaseHandler<'p, P>
where
    P: 'p + Send + Persistence,
{
    type Item = ();
        ...
        match self.state {
            Some(Processing(_, _)) => {
                ...
                if data.server_publish_state.contains_key(&id) {
                    ...
                    let _ = requester.unbounded_send(LoopRequest::Internal(
                        MqttPacket::pub_comp_packet(*id),
                    ));
                } else {
