...
pub struct SubscribeAckHandler<'p, P>
where
...
impl<'p, P> SubscribeAckHandler<'p, P>
where
    P: 'p + Send + Persistence,
{
    pub fn new(
...
impl<'p, P> Future for SubscribeAckHandler<'p, P>
where
    P: 'p + Send + Persistence,
{
    type Item = ();
                ...
                for (sub, ret) in orig.iter().zip(ret_codes.iter()) {
                    if ret.is_ok() {
                        ...
                        data.subscriptions.insert(
                            sub.topic.clone().into(),
                            (filter, tx),
                        );
                        collect.push(Ok((custom_rx.boxed(), sub.qos)));
                    } else {
                        collect.push(Err(
                            ErrorKind::from(ProtoErrorKind::SubscriptionRejected(
                                sub.topic.clone().into(),
                                sub.qos,
                            )).into(),
                        ));
